...
//---------------------------------------------------------------------------------------------------------------------
Speaker::Speaker(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */, uint8_t i2sPort)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setBufsize(int rambuf_sz, int psrambuf_sz)
{
    if (InBuff.isInitialized())
    {
        log_e("Speaker::setBufsize must not be called after audio is initialized");
        return;
    }
    InBuff.setBufsize(rambuf_sz, psrambuf_sz);
};

void Speaker::initInBuff()
{
    if (!InBuff.isInitialized())
    {
...
//---------------------------------------------------------------------------------------------------------------------
esp_err_t Speaker::I2Sstart(uint8_t i2s_num)
{
...
esp_err_t Speaker::I2Sstop(uint8_t i2s_num)
{
    return i2s_stop((i2s_port_t)i2s_num);
}
//---------------------------------------------------------------------------------------------------------------------
esp_err_t Speaker::i2s_mclk_pin_select(const uint8_t pin)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setConnectionTimeout(uint16_t timeout_ms, uint16_t timeout_ms_ssl)
{
    if (timeout_ms)
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::connecttohost(const char *host, const char *user, const char *pwd)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::httpPrint(const char *host)
{
    // user and pwd for authentification only, can be empty
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setFileLoop(bool input)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::UTF8toASCII(char *str)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::connecttospeech(const char *speech, const char *lang)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::urlencode(char *buff, uint16_t buffLen, bool spacesOnly)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::showID3Tag(const char *tag, const char *value)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::unicode2utf8(char *buff, uint32_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::latinToUTF8(char *buff, size_t bufflen)
{
...
//---------------------------------------------------------------------------------------------------------------------
size_t Speaker::readAudioHeader(uint32_t bytes)
{
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::read_WAV_Header(uint8_t *data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::read_FLAC_Header(uint8_t *data, size_t len)
{
    static size_t headerSize;
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::read_ID3_Header(uint8_t *data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::read_M4A_Header(uint8_t *data, size_t len)
{
    /*
           ftyp
...
//---------------------------------------------------------------------------------------------------------------------
size_t Speaker::process_m3u8_ID3_Header(uint8_t *packet)
{
    uint8_t ID3version;
    size_t id3Size;
...
// }
//----------------------------------------------------------------------------------------------------------------------
// const char *Speaker::parsePlaylist_M3U()
// {
...
// }
//----------------------------------------------------------------------------------------------------------------------
// const char *Speaker::parsePlaylist_PLS()
// {
//     uint8_t lines = m_playlistContent.size();
//     int pos = 0;
//     char *host = nullptr;
...
// }
// //----------------------------------------------------------------------------------------------------------------------
// const char *Speaker::parsePlaylist_ASX()
...
// }
// //----------------------------------------------------------------------------------------------------------------------
// const char *Speaker::parsePlaylist_M3U8()
// {
//     uint8_t lines = m_playlistContent.size();
//     bool f_begin = false;
//     uint8_t occurence = 0;
...
// }
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::STfromEXTINF(char *str)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::processWebStreamTS()
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::processWebStreamHLS()
{
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::parseHttpResponseHeader()
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::initializeDecoder()
{
    uint32_t gfH = 0;
    uint32_t hWM = 0;
    switch (m_codec)
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::parseContentType(char *ct)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::showstreamtitle(const char *ml)
{
    // example for ml:
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::findNextSync(uint8_t *data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int Speaker::sendBytes(uint8_t *data, size_t len)
{
    int bytesLeft;
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::compute_audioCurrentTime(int bd)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::printDecodeError(int r)
{
    const char *e;
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN, int8_t MCK)
{
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::getAudioDataStartPos()
{
    if (!audiofile)
        return 0;
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::getAudioFileDuration()
{
    if (getDatamode() == AUDIO_LOCALFILE)
    {
        if (!audiofile)
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::getAudioCurrentTime()
{ // return current time in seconds
    return (uint32_t)m_audioCurrentTime;
}
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setAudioPlayPosition(uint16_t sec)
{
    if (m_codec == CODEC_OPUS)
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setVolumeSteps(uint8_t steps)
{
    m_vol_steps = steps;
    if (steps < 1)
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setTimeOffset(int sec)
{
    if (m_codec == CODEC_OPUS)
        return false; // not impl. yet
    if (m_codec == CODEC_VORBIS)
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setFilePos(uint32_t pos)
{
    if (m_codec == CODEC_OPUS)
        return false; // not impl. yet
    if (m_codec == CODEC_VORBIS)
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::audioFileSeek(const float speed)
{
    // 0.5 is half speed
    // 1.0 is normal speed
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setSampleRate(uint32_t sampRate)
{
    if (!sampRate)
...
uint32_t Speaker::getSampleRate()
{
    return m_sampleRate;
}
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setBitsPerSample(int bits)
{
    if ((bits != 16) && (bits != 8))
        return false;
...
uint8_t Speaker::getBitsPerSample()
{
    return m_bitsPerSample;
}
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setChannels(int ch)
{
    if ((ch < 1) || (ch > 2))
        return false;
    m_channels = ch;
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::setBitrate(int br)
{
    m_bitRate = br;
    if (br)
        return true;
...
uint32_t Speaker::getBitRate(bool avg)
{
    if (avg)
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setI2SCommFMT_LSB(bool commFMT)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::computeVUlevel(int16_t sample[2])
{
...
//---------------------------------------------------------------------------------------------------------------------
bool Speaker::playSample(int16_t sample[2])
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setTone(int8_t gainLowPass, int8_t gainBandPass, int8_t gainHighPass)
{
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::forceMono(bool m)
{                      // #100 mono option
    m_f_forceMono = m; // false stereo, true mono
}
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setBalance(int8_t bal)
{ // bal -16...16
    if (bal < -16)
        bal = -16;
    if (bal > 16)
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::setVolume(uint8_t vol, uint8_t curve)
...
//---------------------------------------------------------------------------------------------------------------------
uint8_t Speaker::getVolume()
{
    return m_vol;
}
//---------------------------------------------------------------------------------------------------------------------
uint8_t Speaker::getI2sPort()
{
    return m_i2s_num;
}
//---------------------------------------------------------------------------------------------------------------------
void Speaker::computeLimit()
...
int32_t Speaker::Gain(int16_t s[2])
{
    int32_t v[2];
...
//---------------------------------------------------------------------------------------------------------------------
void Speaker::IIR_calculateCoefficients(int8_t G0, int8_t G1, int8_t G2)
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *Speaker::IIR_filterChain0(int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *Speaker::IIR_filterChain1(int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *Speaker::IIR_filterChain2(int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
...
//----------------------------------------------------------------------------------------------------------------------
bool Speaker::ts_parsePacket(uint8_t *packet, uint8_t *packetStart, uint8_t *packetLength)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint16_t Speaker::readMetadata(uint16_t maxBytes, bool first)
{
...
//----------------------------------------------------------------------------------------------------------------------
size_t Speaker::chunkedDataTransfer(uint8_t *bytes)
{
...
//----------------------------------------------------------------------------------------------------------------------
boolean Speaker::streamDetection(uint32_t bytesAvail)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::m4a_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::flac_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t Speaker::mp3_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint8_t Speaker::determineOggCodec(uint8_t *data, uint16_t len)
{
